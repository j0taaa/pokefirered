...
- 🚧 Browser port is now at an early playable prototype: map rendering, keyboard movement, collisions, Route 1, Pallet Town, and Viridian City adapter-data loading, metatile behavior metadata, ledge jumps, Route 1/Pallet/Viridian map switching, object-event hide flags, item-script state, encounter-tile-gated wild battles, and runtime tests are in `ts-game/`.
...
11. 🟡 Data convergence with decomp exports (Route 1 + Pallet Town + Viridian City adapter data and parity tests)
