...
describe('Trainer Tower records and e-Reader branches', () => {
  test('Trainer Tower hooks update persisted record vars', () => {
    ...
    expect(finishTrainerTowerChallenge(save, 'single', 612, false).bestTimeSeconds).toBe(612);
    expect(finishTrainerTowerChallenge(save, 'single', 900, false).bestTimeSeconds).toBe(612);
    expect(finishTrainerTowerChallenge(save, 'single', 500, true).playerLost).toBe(true);

    clearTrainerTowerResults(save);
    expect(createTrainerTowerRecordBoard(save, 'single').bestTimeSeconds).toBeNull();
    expect(save.newGame.trainerTowerResultsCleared).toBe(true);
  });

  test('e-Reader send/receive and unsupported checksum branches are deterministic', () => {
    ...
    expect(transferEReaderData(sender, senderOpen.sessionId, payload, 'send').message)
      .toBe(LINK_MESSAGES.eReaderSent);
    ...
    expect(transferEReaderData(sender, senderOpen.sessionId, {
      ...
      words: [1, 2],
      checksum: 99
    }, 'send')).toMatchObject({ status: 'checksumError' });
  });
});
